...
metadata
{
	...
	preferences {
		...
		input name: "debuglogs", type: "bool", title: "Show debug logs", description: "true or false", required: true, displayDuringSetup: true
...
def AlarmDisarm()
{
	...
	if (settings.debuglogs) log.debug "disarm()"
	sendEvent(name: "armaway", value: "inactive", displayed: false)
	sendEvent(name: "armhome", value: "inactive", displayed: false)
...
def ChildDevicesRemove()
{
	...
	try
	{
		getChildDevices()?.each
		{
			try
			{
                if (settings.debuglogs) log.debug "Removing ${it.deviceNetworkId} child device"
...
{
    //Update child switch to show it as on
    ...
    switch (child.deviceNetworkId) {
        ...
        default:
            log.debug "Unknown child device?!"
            break
    }
}
def delayedComponentOff(childname)
{
    def childdevice = getChildDevices()?.find { it.deviceNetworkId == childname }
    childdevice?.sendEvent(name: "switch", value: "off")
}
def componentOff(child)
{
    //Do nothing unless it is an output that we need to switch
    child?.sendEvent(name: "switch", value: "off")
    if (child.deviceNetworkId ==~/alarmchildoutput.*/)
    {
        if (settings.debuglogs) log.debug "childOff(${child.deviceNetworkId})"
        def outputnumber = dni.replaceAll("alarmchildoutput", "")
...
{
    if (settings.debuglogs) log.debug "childRefresh(${child.deviceNetworkId})"
    refresh()
}
...
def configure()
{
	...
	if (settings.inactivityseconds?.isInteger())
	{
...
def refresh()
{
	if (settings.debuglogs) log.debug "refresh()"
	getAction("/refresh")
}
...
def ping()
{
	if (settings.debuglogs) log.debug "ping()"
	getAction("/ping")
}
	...
	if (result.containsKey("update_type"))
	{
		switch (result.update_type)
		{
			...
			case ["create_zones", "zone_names"]:
				if (settings.debuglogs) log.debug "Handling getzonenames page"
				handleCreateZones(result)
				break

			case ["remove_zones"]:
				if (settings.debuglogs) log.debug "Removing zones where possible as received command from bridge"
				ChildDevicesRemove()
				break
		}
...
private handleRefresh(result)
{
	...
	{
		handleZoneStatus(curzone) //needs zone_id and zone_status
	}
}
	...
	switch (result.stat_str)
	{
		case ["Disarmed", "Disarm", "Ready"]:
			...
			if (settings.changehsm) {
				if (settings.debuglogs) log.debug "Updating HSM"
		...
		case ["Exit Delay"]:
			...
			if (settings.debuglogs) log.debug "Exit Delay Status found"
			break

		case ["Delay Alarm", "Confirm Alarm", "Perimeter Alarm", "Interior Alarm", "Tamper Alarm", "24h Silent Alarm", "24h Audible Alarm", "Panic From Keyfob", "Panic From Control Panel", "Duress"]:
			sendEvent(name: "disarm", value: "inactive", displayed: false)
			sendEvent(name: "armaway", value: "inactive", displayed: false)
			sendEvent(name: "armhome", value: "inactive", displayed: false)
...
private handleZoneStatus(result)
{
	...
	else
	{
		...
		switch (result.zone_status)
		{
			...
			case ["No Motion", "Inactive", "Not Active"]:
			...
			case "Open":
				log.debug "Open zone: " + result.zone_id + ", which is called - " + curdevice
				if (isMotionDevice)
				{
			...
			case "Closed":
				log.debug "Closed zone: " + result.zone_id + ", which is called - " + curdevice
				if (isMotionDevice)
				{
			...
			case ["Bypassed (Motion)", "Bypassed - Active"]:
				log.debug "Active Bypassed zone: " + result.zone_id + ", which is called - " + curdevice
				if (isMotionDevice)
				{
					curdevice?.sendEvent(name: "motion", value: "active") // Correct message
				}
			...
			case ["Bypassed", "Bypassed - Inactive"]:
				log.debug "Inactive Bypassed zone: " + result.zone_id + ", which is called - " + curdevice
				if (isMotionDevice)
				{
					curdevice?.sendEvent(name: "motion", value: "inactive") // Correct message
				}
			...
			case "Tamper":
				log.debug "Tamper for zone: " + result.zone_id + ", which is called - " + curdevice
...
private handleCreateZones(result)
{
	...
	for (def curzone in result.zones)
	{
		...
		thisname = thisname + curzone.zone_name
		...
		{
			if (curzone.zone_name != "Unknown")
			{
				try
				{
					switch (curzone.zone_type)
					{
						case ["Magnet", "Contact", "Entry/Exit"]:
							// If it is a magnetic sensor then add it as a contact sensor
							addChildDevice("hubitat", "Generic Component Contact Sensor", "alarmchildzone${thisid}", [name: thisname])
							log.debug "Creating contact zone with name: ${thisname}, ID: alarmchildzone${thisid} to Hub ${hub.id}"
							break

						case ["Motion", "Interior", "Wired"]:
							// If it is a motion sensor then add it as a motion detector
							addChildDevice("hubitat", "Generic Component Motion Sensor", "alarmchildzone${thisid}", [name: thisname])
							log.debug "Creating motion zone with name: ${thisname}, ID: alarmchildzone${thisid} to Hub ${hub.id}"
							break

						case ["Smoke", "Fire"]:
							// If it is a smoke or fire sensor then add it as a smoke detector
							addChildDevice("hubitat", "Generic Component Smoke Detector", "alarmchildzone${thisid}", [name: thisname])
							log.debug "Creating smoke zone with name: ${thisname}, ID: alarmchildzone${thisid} to Hub ${hub.id}"
							break
						...
						case ["Shock", "Vibration", "Gas", "Panic", "KeySwitch"]:
							...
							addChildDevice("hubitat", "Generic Component Motion Sensor", "alarmchildzone${thisid}", [name: thisname])
							log.debug "Creating motion zone instead of shock/vibration/gas/panic with name: ${thisname}"
							break

						case ["Output"]:
							// This is an output zone for controlling on/off
							addChildDevice("hubitat", "Generic Component Switch", "alarmchildoutput${thisid}", [name: "${thisname} Output"])
							log.debug "Creating output zone with name: ${thisname}, ID: alarmchildoutput${thisid} to Hub ${hub.id}"
							break

						default:
							log.debug "Unknown sensor found, we'll ignore for now....?!"
							break
					}
				}
				catch (e)
    ...
    if (settings.createcontrolchilddevices) {
        ...
        switches.each {
            try {
                def child = addChildDevice("hubitat", "Generic Component Switch", "alarm${it}", [name: "Alarm ${it.capitalize()}"])
                child.off()
            }
            catch (e)
			{
...
private handleOutputStatus(result)
{
	...
	else
	{
        ...
        curdevice?.sendEvent(name: "switch", value: result.output_status.toLowerCase())
	}
}

private getAction(uri)
{ 
	if (settings.debuglogs) log.debug "Get Action uri = ${uri}"
	updateDNI()
...
{ 
	if (settings.debuglogs) log.debug "Post Action uri = ${uri}"
	updateDNI()
...
private outputon(outputnumber, period = 0)
{
	...
	if (settings.debuglogs) log.debug "outputon()"
...
{ 
	...
	if (settings.debuglogs) log.debug "outputoff()"
